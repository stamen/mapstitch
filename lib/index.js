...
var getTilesForView = stitch.getTilesForView = function(view) {
  if (view.extent[2] > 180) {
    var left = merc.xyz([view.extent[0], view.extent[1], 180, view.extent[3]], view.zoom);
    var right = merc.xyz([-180, view.extent[1], view.extent[2] - 360, view.extent[3]], view.zoom);

    var leftRange = d3.range(left.minX, left.maxX + 1).map(function(x) {
      // tileRange.*Y is backward because our y coordinates start at the top
      return d3.range(left.maxY, left.minY + 1).map(function(y) {
        return {
          zoom: view.zoom,
          x: x,
          y: y
    ...
    var rightRange = d3.range(right.minX, right.maxX + 1).map(function(x) {
      // tileRange.*Y is backward because our y coordinates start at the top
      return d3.range(right.maxY, right.minY + 1).map(function(y) {
        return {
          zoom: view.zoom,
          x: x,
          y: y
...
var crop = stitch.crop = function(canvas, view, width, height) {
  ...
  targetCtx.drawImage(canvas, startX, startY, canvas.width, canvas.height,
                              0, 0, width, height);
